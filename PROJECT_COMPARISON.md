...
| Local HTTP API | ✅ 19828端口 | ✅ server-rs Rust后端 | 增强 |
...
| Agent Skill 暴露（19828 HTTP API） | ✅ | ⚠️ 有 server-rs 但未完整对接 | P0 |
...
我们有 server-rs 后端，但 `expandGraphFromEntity()` 没有通过 HTTP 暴露出去。nashsu 已经解决了这个问题，我们可以直接参考其 API 设计。
