...
`desktop/src/commands/fs.rs::preprocess_file`:
...
New commands in `desktop/src/commands/fs.rs` (or a new
`desktop/src/commands/extract_images.rs` if `fs.rs` is getting too
...
Tests (`desktop/src/commands/extract_images.rs::tests`):
...
`frontend/src/lib/llm-providers.ts`:
...
`frontend/src/lib/vision-caption.ts` (new):
...
`frontend/src/lib/ingest.ts` integration:
...
`frontend/src/components/settings/sections/embedding-section.tsx` (or a new
