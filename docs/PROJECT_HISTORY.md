...
### 核心文件路径映射
```
frontend/src/lib/knowledge-schema.ts          # 通用 schema 类型定义、domain 枚举、关系类型
frontend/src/lib/insurance-schema-registry.ts # 保险行业实体规格（schema key → 字段清单）
frontend/src/lib/ingest.ts                    # 文档摄入 pipeline（OCR → intent识别 → LLM抽取 → wiki写入）
frontend/src/lib/knowledge-schema-normalizer.ts # frontmatter 标准化
frontend/src/lib/knowledge-postprocess.ts     # 摄入后处理（关系推断、身份解析）
frontend/src/lib/knowledge-global-relation.ts # 全局关系 pass
frontend/src/lib/knowledge-identity-resolution.ts # 跨文档实体去重/合并
frontend/src/lib/service-benefit-enrichment.ts   # 服务权益专项富化
frontend/src/lib/insurance-schema-registry.ts    # 服务层级注册（系列>场景>线>版本>服务项）
backend/src/handlers/fs.rs         # Rust 后端文件系统 + PDF 读取（4级 OCR fallback）
```
...
**变更（`frontend/src/lib/knowledge-schema.ts`）**：
...
**变更（`frontend/src/lib/insurance-schema-registry.ts`）**：
...
### 5. PDF 处理 4 级 fallback（`backend/src/handlers/fs.rs`）
```
...
- `backend/src/state.rs` / `backend/src/handlers/fs.rs` / `backend/src/main.rs`
...
**本地联调结果**：
- 修复本地 Web 服务启动后 `/api/config` 模型配置为空的问题：Rust 服务会依次检查当前目录、`backend/.env` 与可执行文件祖先目录，只补充尚未由系统环境变量提供的配置。
...
- 新增 `deployment/releases/llm-wiki-linux-amd64-docker20/DEPLOYMENT.md`，覆盖 `.env`、模型/OCR 配置、网络、验证、备份与排障流程。
...
- 新增 `deployment/releases/llm-wiki-linux-amd64-docker20/API_REFERENCE.md`，梳理 39 条前后端 API、外部模型协议、业务调用链和当前安全/可观测性缺口。
- 容器 smoke test 通过：首页 200、`/api/health` 正常、运行时 LLM/Vision/Embedding 配置可读取。

**交付目录**：`deployment/releases/llm-wiki-linux-amd64-docker20/`
...
### [2026-07-02] v3.0 大规模 Markdown 展示与后台精炼

**问题**：累计导入数百个源文件后会生成数千个产品主文件、模块页、字段页和 concept。前端曾重复递归扫描目录、逐页读取元数据，并在搜索时一次渲染全部结果；全局精炼还在浏览器中读取所有产品 MD、维持 10 路 LLM 请求并重建全库，数据量增长后会造成明显卡顿且页面关闭即中断。

**解决方案**：
- 项目文件树改为按 `dataVersion` 单入口防抖刷新，旧请求结果不会覆盖新目录；知识树复用已有文件树，不再重复请求整个项目。
- 移除队列运行期间每 2.5 秒扫描产品库；批次和精炼完成后统一通过 SSE 触发一次刷新。
- 页面、搜索结果和 Raw Sources 每次最多渲染 100 行，可按需继续加载；产品搜索不再强制展开所有产品层级。
- 新增后台精炼 API：`POST /api/ingest/product-refinements`、`GET /api/ingest/product-refinements/{job_id}`。任务状态持久化，浏览器刷新/关闭不影响执行；重复点击会恢复现有活动任务。
- 精炼与产品导入复用全局 worker 限流和项目级互斥锁，避免同时改写同一项目 Markdown。
- `REFINE_PARALLEL` 独立控制二次精炼并发，默认从硬编码 10 收敛为 4，范围 `1~16`。
- 普通精炼只补空字段，不重复抽取已有值；补齐模块定义中的缺失关键字段和 rejected 字段页。
- 文件识别先按模块/字段文件名缩小候选集，避免读取无关产品主文件；完成后只重建实际变化的产品，再同步 concepts。
- 精炼任务每 30 秒更新心跳，服务异常遗留的旧任务不会永久阻止新任务。

**处理效果**：普通浏览不再传输全部产品 MD 正文，列表 DOM 数量保持有界；精炼的文件读取、LLM 请求和 Markdown 重建转移到后端，前端保持可操作。当前方案适合数千至数万 Markdown；更大规模可在现有接口之上继续增加持久化页面元数据索引和服务端分页。

**验证**：前端 Vite 生产构建、worker SSR 构建、Rust `cargo check` 通过；全项目 TypeScript 检查仍有 46 个历史错误，本轮修改文件未新增错误。
