//#region src/lib/insurance-schema-registry.ts
var f = (name, label, note, importance) => ({
	name,
	label,
	note,
	importance
});
var DEDUP_KEY_FIELDS = {
...
var FIELD_ALIASES = {
	...
	success_case: {
		customer_profile_brief: [
			"customer_profile",
			"客户画像",
			"客户背景"
		],
		product_or_combo_sold: [
			"product",
			"service",
			"关联产品",
			"关联服务"
...
function stripBrandSuffix(title) {
	...
	for (const pattern of BRAND_SUFFIX_PATTERNS) result = result.replace(pattern, "");
	return result.trim();
}
/**
...
*/
function canonicalServiceIdentityName(title) {
	...
	if (fromDiscriminators.some((d) => !toSet.has(d)) || toDiscriminators.some((d) => !fromSet.has(d))) return stripped;
	return canonical;
}
var INSURANCE_SCHEMA_REGISTRY = [
	{
	...
	},
	{
		...
		lintRules: ["plan_name 和 plan_version 必须稳定，是 dedup_key 的基础。", "has_part 关系应覆盖全部 service_benefit 子页面。"]
	},
	{
	...
	},
	{
		...
		relationHints: [
			"related_product 用 applies_to",
			"target_persona 用 recommended_for",
			"高敏感卖点用 governed_by"
		]
	},
	{
	...
	},
	{
		...
		relationHints: ["service_scenario 子节点用 has_part"]
	},
	{
		...
		relationHints: ["service_line 子节点用 has_part", "所属 service_series 用 part_of"]
	},
	{
		...
		relationHints: ["service_line_version 子版本用 has_part", "所属 service_scenario 用 part_of"]
	},
	{
		...
		lintRules: ["line_name 和 version_name 是 DEDUP_KEY，一旦确定不能随意修改。", "has_part 关系应覆盖该版本下所有 service_item 实体。"]
	},
	{
	...
	},
	{
		...
		relationHints: ["各版本具体实例 service_item 用 has_instance（反向为 instance_of）"],
		lintRules: ["聚合页只写通用定义，不写具体版本的次数/城市/限制。", "has_instance 关系应覆盖所有已知版本实例。"]
	},
	{
	...
	},
	{
		...
		relationHints: [
			...
			"Product 用 recommended_for",
			"SellingScenario 用 applies_to"
		]
	},
	{
	...
	},
	{
		...
		relationHints: [
			"Persona 用 applies_to",
			"ProductCombo/Product 用 applies_to",
			"Pitch/Asset 用 supports 或 uses_asset"
		]
	},
	{
		...
		relationHints: [
			"SellingScenario 用 has_part",
			"Campaign 用 applies_to",
			"Content.Asset 用 uses_asset"
		]
	},
	{
	...
	},
	{
		...
		relationHints: [
			"Asset 用 has_part",
			"Campaign/SalesPlaybook 用 applies_to",
			"Compliance.Rule 用 governed_by"
		]
	},
	{
	...
	},
	{
		...
		relationHints: [
			"Asset 用 has_part",
			"SalesPlaybook/Campaign 用 uses_asset",
			"Compliance.Rule 用 governed_by"
		]
	},
	{
		...
		relationHints: [
			"SalesPlaybook/SellingScenario 用 applies_to",
			"Asset/PresentationKit 用 uses_asset",
			"Incentive 用 has_part"
		]
	},
	{
	...
	},
	{
		...
		relationHints: [
			"Campaign 用 part_of",
			"Asset/PresentationKit 用 uses_asset",
			"Product/Persona/SellingScenario 用 applies_to"
		]
	},
	{
...
function renderInsuranceSchemaRegistryPrompt() {
	const lines = [
	...
	for (const spec of INSURANCE_SCHEMA_REGISTRY) {
		...
		for (const importance of [
			...
			for (const field of fields) lines.push(`  - ${field.name}: ${field.label}；${field.note}`);
		}
...
function getInsuranceSchemaSpec(entityType) {
...
function getInsuranceFieldMergePolicy(entityType, fieldName) {
	const field = getInsuranceSchemaSpec(entityType)?.fields.find((item) => item.name === fieldName);
	if (!field) return "conflict";
	if (field.importance === "recommended") return "append";
	if (field.importance === "auto_derived") return "keep_best";
	return "conflict";
}
function getInsuranceFieldImportance(entityType, fieldName) {
	return getInsuranceSchemaSpec(entityType)?.fields.find((item) => item.name === fieldName)?.importance;
}
function getCanonicalInsuranceFieldName(entityType, fieldName) {
	const normalizedEntityType = normalizeToken(entityType);
	const normalizedField = normalizeToken(fieldName);
	if (getInsuranceSchemaSpec(normalizedEntityType)?.fields.some((field) => field.name === normalizedField)) return normalizedField;
	const aliases = FIELD_ALIASES[normalizedEntityType] ?? {};
	for (const [canonical, values] of Object.entries(aliases)) if (values.some((alias) => normalizeToken(alias) === normalizedField)) return canonical;
	return normalizedField;
}
function normalizeInsuranceAttributes(entityType, attributes) {
	const normalized = {};
	for (const [field, value] of Object.entries(attributes)) {
		const canonical = getCanonicalInsuranceFieldName(entityType, field);
		if (normalized[canonical] === void 0) normalized[canonical] = value;
		else if (Array.isArray(normalized[canonical])) normalized[canonical] = [...normalized[canonical], value];
		else normalized[canonical] = [normalized[canonical], value];
	}
	return normalized;
}
function inferStableInsuranceDedupKey(input) {
	...
	for (const field of keyFields) {
		...
		if (IDENTITY_NORMALIZE_FIELDS.has(field) && typeof value === "string") value = canonicalServiceIdentityName(value);
...
function inferStrongInsuranceIdentityKeys(input) {
	const entityType = normalizeToken(input.entityType || "general");
	...
	if (entityType === "product") {
		...
		if (titleKey) keys.add(`${entityType}.title.${titleKey}`);
	} else if (entityType === "selling_scenario") addFieldKey("scenario_name", "name");
	else if (entityType === "pitch") addCompositeKey([
		"related_scenario",
		"pitch_type",
		"core_message"
	], "scenario_type_message");
	else if (entityType === "objection_handling") addCompositeKey(["objection_category", "objection_raw"], "category_raw");
	else {
...
function inferSourceTypeFromSourceName(sourceName = "") {
...
function normalizeDedupPart(value) {
	...
	return (Array.isArray(value) ? value.join("_") : String(value)).trim().toLowerCase().replace(/[\s/\\|,，、;；:：()[\]{}'"“”‘’]+/g, "_").replace(/[^\p{L}\p{N}_-]+/gu, "").replace(/^_+|_+$/g, "").slice(0, 80);
}
function normalizeBusinessTitle(value, entityType) {
	let title = String(value ?? "").trim().toLowerCase();
	if (!title) return "";
	title = title.replace(/\.(md|pdf|docx?|xlsx?|png|jpe?g)$/i, "").replace(/[\s/\\|,，、;；:：()[\]{}'"“”‘’]+/g, "");
	if (entityType === "product") title = title.replace(/重大疾病/g, "重疾").replace(/重疾险/g, "重疾").replace(/产品说明书|产品说明|说明书|合同条款|保险条款|条款|宣传单页|宣传材料|费率表|准入清单|清单|pdf|文档/g, "").replace(/保险产品|产品$/g, "").replace(/保险$/g, "");
	else if (entityType === "service_benefit") title = title.replace(/服务权益名称|服务名称|权益名称|服务项目名称/g, "").replace(/服务权益|权益服务|服务项目|服务说明|流程说明|说明|规则|pdf|文档/g, "");
	else if (entityType === "service_item") title = title.replace(/服务项名称|服务项目名称|服务项目|服务名称/g, "").replace(/pdf|文档|说明/g, "");
	else if (entityType === "persona") title = title.replace(/客户画像|画像|客户|人群/g, "");
	return title.length >= 4 ? normalizeDedupPart(title) : "";
}
var SERVICE_HIERARCHY = [{
	seriesName: "添平安系列",
	status: "active",
	scenarios: [
		{
			...
			lines: [
				...
				{
					lineName: "安有护",
					versions: [{ versionName: "国际" }, { versionName: "国内" }]
				},
				{
					lineName: "就医通",
					versions: [{ versionName: "就医通" }]
				},
				{
					lineName: "臻享家医",
					versions: [
				...
				{
					lineName: "御享国医",
					versions: [{ versionName: "御享国医" }]
				},
				{
					lineName: "私董保健医",
					versions: [{ versionName: "京华版" }, { versionName: "繁华版" }]
				}
			]
		},
		{
...
*   → "安有医-颐享版-家庭医生服务"
*/
function buildServiceItemTitle(lineName, versionName, itemName) {
...
*/
function parseServiceItemTitle(title) {
...
*/
function resolveCanonicalVersionName(lineName, versionName) {
	...
	if (lineAliases) {
		...
		for (const [alias, canonical] of Object.entries(lineAliases)) if (alias.toLowerCase() === lower) return canonical;
	}
	return versionName;
}
/**
...
*/
function findServiceLineVersion(lineName, versionName) {
	const canonical = resolveCanonicalVersionName(lineName, versionName);
	for (const series of SERVICE_HIERARCHY) for (const scenario of series.scenarios) for (const line of scenario.lines) if (line.lineName === lineName) {
		const version = line.versions.find((v) => v.versionName === canonical || v.versionName === versionName);
		if (version) return {
			series: series.seriesName,
			scenario: scenario.scenarioName,
			canonicalVersionName: version.versionName
		};
	}
	return null;
}
var PRODUCT_CATALOG_SCHEMA = [
	{
	...
	},
	{
		...
		relationHints: ["related_product 用 part_of 指向 product_overview", "Compliance.RegulatoryDoc 用 governed_by"],
		lintRules: [
...
//#endregion
export { findServiceLineVersion as a, getInsuranceSchemaSpec as c, inferStrongInsuranceIdentityKeys as d, normalizeInsuranceAttributes as f, sourceTypeWeight as h, canonicalServiceIdentityName as i, inferSourceTypeFromSourceName as l, renderInsuranceSchemaRegistryPrompt as m, SERVICE_HIERARCHY as n, getInsuranceFieldImportance as o, parseServiceItemTitle as p, buildServiceItemTitle as r, getInsuranceFieldMergePolicy as s, INSURANCE_SCHEMA_REGISTRY as t, inferStableInsuranceDedupKey as u };
