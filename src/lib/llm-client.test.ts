...
import { isFetchNetworkError, toOpenAiCompatMessages } from "./llm-client"
...
describe("toOpenAiCompatMessages", () => {
  it("converts internal image blocks to OpenAI-compatible image_url blocks", () => {
    const out = toOpenAiCompatMessages([
      {
        role: "user",
        content: [
          { type: "text", text: "提取文字" },
          { type: "image", mediaType: "image/jpeg", dataBase64: "abc123" },
        ],
      },
    ])
  ...
  it("preserves legacy string messages", () => {
    expect(toOpenAiCompatMessages([{ role: "user", content: "hello" }])).toEqual([
