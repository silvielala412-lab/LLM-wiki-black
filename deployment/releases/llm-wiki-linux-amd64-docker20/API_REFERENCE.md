...
| 前端文件 | 负责的 API |
|---|---|
| `frontend/src/commands/api-client.ts` | 文件、项目、上传、向量接口的统一封装 |
| `frontend/src/commands/fs.ts` | 对 `api-client.ts` 的兼容性重导出 |
| `frontend/src/stores/auth-store.ts` | 注册、登录、退出、当前用户 |
| `frontend/src/lib/server-config.ts` | 获取服务端模型配置 `/api/config` |
| `frontend/src/components/chat/chat-panel.tsx` | 项目问答 `/api/chat/stream`、RAG 状态 |
| `frontend/src/lib/llm-client.ts` | 主 LLM 与 OpenAI 兼容视觉模型代理 |
| `frontend/src/lib/embedding.ts` | Embedding 代理 |
| `frontend/src/lib/web-search.ts` | Web Search 代理 |
