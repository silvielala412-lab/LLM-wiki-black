import { a as listDirectory, n as createDirectory, o as readFile, u as writeFile } from "./fs-WYeR_9ZT.js";
import { normalizePath } from "./path-utils-BHuw7z0q.js";
import { p as parseServiceItemTitle } from "./insurance-schema-registry-AHUYvTTg.js";
import { INSURANCE_CATEGORIES, isModuleAllowedForCategory, parseProductModuleTitle } from "./product-catalog-modules-Bd1IGcTo.js";
//#region src/lib/concept-aggregator.ts
/**
...
function flattenMdFiles(nodes) {
	const files = [];
	for (const n of nodes) if (n.is_dir && n.children) files.push(...flattenMdFiles(n.children));
	else if (!n.is_dir && n.name.endsWith(".md")) files.push(n);
	return files;
}
async function scanServiceItemConcepts(projectPath) {
	const pp = normalizePath(projectPath);
	const conceptMap = /* @__PURE__ */ new Map();
	let entityFiles = [];
	try {
		entityFiles = flattenMdFiles(await listDirectory(`${pp}/wiki/entities`));
	} catch {
		return conceptMap;
	}
	for (const file of entityFiles) try {
		const fmMatch = (await readFile(file.path)).match(/^---\n([\s\S]*?)\n---/);
		if (!fmMatch) continue;
		...
		const relPath = file.path.includes(pp) ? file.path.slice(pp.length).replace(/^[/\\]/, "") : file.path;
...
function buildServiceConceptPage(itemName, instances) {
	const sorted = [...instances].sort((a, b) => a.lineName.localeCompare(b.lineName) || a.versionName.localeCompare(b.versionName));
...
async function buildServiceConceptIndex(projectPath) {
	const pp = normalizePath(projectPath);
	...
	for (const [itemName, instances] of conceptMap) {
		...
		try {
			...
			if (existing === null) {
				...
				if ((existingCountMatch ? parseInt(existingCountMatch[1]) : 0) !== instances.length) {
					await writeFile(conceptPath, newContent);
					updated++;
				} else skipped++;
			}
...
async function backfillEntityRelated(projectPath) {
	const pp = normalizePath(projectPath);
	const conceptMap = await scanServiceItemConcepts(projectPath);
	...
	for (const [, instances] of conceptMap) {
		...
		for (const inst of instances) {
			...
			try {
				...
				const existing = existingMatch ? existingMatch[1].split(",").map((s) => s.trim().replace(/^["']|["']$/g, "")).filter(Boolean) : [];
				const newRelatedLine = `related: [${Array.from(new Set([...existing, ...siblings])).map((t) => `"${t}"`).join(", ")}]`;
				const newContent = existingMatch ? content.replace(/^related:\s*\[([^\]]*)\]/m, newRelatedLine) : content.replace(/^(---\r?\n[\s\S]*?)(related:\s*\n)/m, (_, pre) => `${pre}${newRelatedLine}\n`);
...
function getProductConceptGroup(conceptMap, conceptName) {
...
var PLACEHOLDER_VALUE_REGEX = /^(未明确|未提及|未提到|未说明|未在本|未在证据|未从证据|证据片段未|证据片段中未|证据片段没有|证据中未|该字段未|文中未|原文未|原文中未|材料未|未找到|未见|没有提到|证据不足|无明确|不涉及|暂无|暂未|无此信息|无相关|本章节未|条款未|不适用于本|N\/A|n\/a|无$)/;
...
*/
async function scanProductConcepts(projectPath) {
	const pp = normalizePath(projectPath);
	const conceptMap = /* @__PURE__ */ new Map();
	let catalogFiles = [];
	try {
		catalogFiles = flattenMdFiles(await listDirectory(`${pp}/wiki/product_catalog`));
	} catch {
		return conceptMap;
	}
	for (const file of catalogFiles) try {
		...
		if (frontmatterScalar(fm, "status") === "rejected") continue;
		...
		if (!INSURANCE_CATEGORIES.includes(category)) continue;
		if (!product) continue;
		const relPath = file.path.includes(pp) ? file.path.slice(pp.length).replace(/^[/\\]/, "") : file.path;
		if (domain === "product_catalog_field") {
			...
			if (!group.fields.some((i) => i.productName === product && i.fieldName === fieldName)) group.fields.push({
		...
		if (!isModuleAllowedForCategory(category, moduleName)) continue;
		const group = getProductConceptGroup(conceptMap, moduleName);
		if (!group.modules.some((i) => i.productName === product && i.moduleName === moduleName)) group.modules.push({
			insuranceCategory: category,
			productName: product,
			moduleName,
...
function buildProductConceptPage(conceptName, group) {
	const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
	const moduleInstances = [...group.modules].sort((a, b) => a.insuranceCategory.localeCompare(b.insuranceCategory) || a.productName.localeCompare(b.productName));
	const fieldInstances = [...group.fields].sort((a, b) => a.insuranceCategory.localeCompare(b.insuranceCategory) || a.productName.localeCompare(b.productName));
	const instanceCount = moduleInstances.length + fieldInstances.length;
	const conceptRelativeLink = (wikiRelPath) => `../${wikiRelPath.replace(/\\/g, "/").replace(/^wiki\//, "")}`;
	...
	const relatedProducts = Array.from(new Set([...moduleInstances.map((i) => `"${i.insuranceCategory}-${i.productName}"`), ...fieldInstances.map((i) => `"${i.insuranceCategory}-${i.productName}"`)])).join(", ");
	const fieldSection = fieldInstances.length > 0 ? `## 字段实体（${fieldInstances.length}个）

| 险种类别 | 产品名称 | 字段范围 | 字段值摘要 | 字段页 |
|---|---|---|---|---|
${fieldRows}` : `## 字段实体\n\n暂无字段页归属此概念。`;
	const moduleSection = moduleInstances.length > 0 ? `## 模块实体（${moduleInstances.length}个）
...
*/
async function buildProductConceptIndex(projectPath) {
	const pp = normalizePath(projectPath);
...
*/
async function runConceptAggregator(projectPath) {
